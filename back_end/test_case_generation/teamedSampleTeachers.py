    ...
    for course in courseToSections.keys():
        ...
        if course in {"Biology 1", "English 9", "Design and Tech"}:
            ...
            for k in range(t):
                ...
                for y in range(3):
                    sections[secID] = [secID.__str__(), "teacher: "+teachers[teacher].split(", ")[2], "courseID: "+courseToNumber[course], "room: ?", "semester: 0","maxstudents: 20", "allowed_periods: 1,2,3"]
                    teacherToSection[teacher].append(secID)
                    secID += 1
                for y in range(3):
                    sections[secID] = [secID.__str__(), "teacher: " + teachers[teacher].split(", ")[2], "courseID: " + courseToNumber[course], "room: ?", "semester: 0", "maxstudents: 20", "allowed_periods: 5,6,7"]
                    teacherToSection[teacher].append(secID)
                    secID += 1
        else:
            for y in range(courseToSections[course]):
                teacher = random.choice(courseToTeacher[course])
                while teacher in teacherToSection.keys() and len(teacherToSection[teacher]) >= 5:
                    teacher = random.choice(courseToTeacher[course])
                if not teacher in teacherToSection.keys():
                    teacherToSection[teacher] = []
                sections[secID] = [secID.__str__(), "teacher: "+teachers[teacher].split(", ")[2], "courseID: "+courseToNumber[course], "room: ?"]
